...
namespace OW.Game.Mission
{
    ...
    public class GameMissionManager : GameManagerBase<GameMissionManagerOptions>
    {
        ...
        {
            _MissionTemplates = new Lazy<List<GameItemTemplate>>(() => World.ItemTemplateManager.GetTemplates(c => c.CatalogNumber == (int)ThingGId.任务成就_成就_成就 / 1000).ToList(),
                    ...
                    case "49ee3541-3a6e-4d05-85b0-566c6bfecde2": //	纯种坐骑数量成就	51006
                        {
                            ...
                            {
                                return gim.IsChunzhongMounts(c);
                            });
                        }
                        break;
                    ...
                    case "26c63192-867a-43f4-919b-10a614ee2865": //	炮塔成就	51017
                        {
                            ...
                            if (coll.Any())
                                metrics = coll.Max(c => c.GetSdpDecimalOrDefault(World.PropertyManager.LevelPropertyName));
                        }
                        break;
