...
namespace GuangYuan.GY001.BLL
{
    ...
    public class GameShoppingManager : GameManagerBase<GameShoppingManagerOptions>
    {
        ...
        public void Buy(BuyDatas datas)
        {
            ...
            else //若非自动使用物品
            {
                ...
                if (list.Count > 0)    //若需要发送邮件
                {
                    ...
                    World.SocialManager.SendMail(mail, new Guid[] { datas.GameChar.Id }, SocialConstant.FromSystemId, list.Select(c => (c, World.EventsManager.GetDefaultContainer(c, datas.GameChar).ExtraGuid)));
                }
            }
            //改写购买记录数据
        ...
        private IEnumerable<(GameItem, decimal)> GetCost(GameShoppingTemplate template, decimal count, GameChar gameChar)
        {
            ...
            if (template.TryGetValue("bd", out _))   //若有钻石售价
            {
            ...
            else
            {
                ...
                if (tt != null && tt.TryGetValue("bd", out _))    //若有基础模板钻石售价
                {
                    result.Add((gameChar.GetZuanshi(), -Math.Abs(tt.GetSdpDecimalOrDefault("bd")) * count));
                }
            }
            if (template.TryGetValue("bg", out _))   //若有金币售价
            {
            ...
            {
                var tt = gitm.GetTemplateFromeId(template.ItemTemplateId ?? Guid.Empty);
                if (tt != null && tt.TryGetValue("bg", out _))    //若有基础模板金币售价
                {
                    result.Add((gameChar.GetJinbi(), -Math.Abs(tt.GetSdpDecimalOrDefault("bg")) * count));
                }
            }
            if (template.TryGetValue("bmg", out _))  //若有公会币售价
            {
            ...
            {
                var tt = gitm.GetTemplateFromeId(template.ItemTemplateId ?? Guid.Empty);
                if (tt != null && tt.TryGetValue("bmg", out _))    //若有基础模板公会币售价
                {
        ...
        void Choujiang1(ChoujiangDatas datas)
        {
            ...
            //计算概率
            var probs = templates.Select(c => (c.Key, c.Value.First(d => d.TryGetValue("prob", out _)).GetSdpDecimalOrDefault("prob")));
        ...
        void UseCardPoolTemplates(ChoujiangDatas datas, IEnumerable<GameCardPoolTemplate> templates)
        {
            ...
            if (remainder.Count > 0)   //若有需要发送邮件得物品
            {
                ...
                World.SocialManager.SendMail(mail, new Guid[] { datas.GameChar.Id },
                    SocialConstant.FromSystemId, remainder.Select(c => (c, World.EventsManager.GetDefaultContainer(c, datas.GameChar).ExtraGuid)));
            }
        }
    ...
    public static class GameShoppingManagerExtensions
    {
        ...
        public static GameItem GetShoppingSlot(this GameChar gameChar) =>
            gameChar.GameItems.FirstOrDefault(c => c.ExtraGuid == ShoppingSlotTId);
        ...
        public static GameItem GetChoujiangquan(this GameChar gameChar) =>
            gameChar.GetItemBag()?.Children?.FirstOrDefault(c => c.ExtraGuid == ProjectConstant.ChoujiangjuanTId);
